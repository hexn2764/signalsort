...
#   ./scripts/reset.sh --yes    # no questions
#
# Closes autopilot pull requests and issues, deletes autopilot branches
# (local and remote), prunes worktrees and clears the agent event log.
# Never touches main, and never touches anything a human opened.
set -uo pipefail
...
pkill -f "src/agents/(reporter|triage|solver).ts" 2>/dev/null && echo "    agents stopped" || echo "    none running"
